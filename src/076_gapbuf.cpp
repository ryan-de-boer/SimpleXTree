...
//https://github.com/mkohlhaas/Pointers-C-Programming/blob/main/09_Dynamic_Memory_Management/076_gapbuf.c
...
{
  GapBuf* buf = (GapBuf*)malloc(sizeof *buf); if (!buf) return NULL;
  init_size = MAX(init_size, MIN_BUF_SIZE);
  buf->buffer = (char*)malloc(init_size); if (!buf->buffer) { free(buf); return NULL; }
  buf->size = init_size;
...
shrink_buffer(GapBuf* buf, size_t new_size)
{
  ...
  char* new_buf = (char*)realloc(buf->buffer, new_size); // allocate a smaller buffer
...
grow_buffer(GapBuf* buf, size_t new_size)
{
  ...
  char* new_buf = (char*)realloc(buf->buffer, new_size); // allocate a larger buffer
...
extract_text(GapBuf* buf)
{
  ...
  char* text = (char*)malloc(gb_used(buf) + 1);
  if (!text) return NULL;
...
char* extractFront(GapBuf* buf)
{
  ...
  char* text = (char*)malloc(gb_front(buf) + 1);
  if (!text) return NULL;

  strncpy(text, buf->buffer, buf->cursor);
  text[gb_front(buf)+1] = '\0';
...
char* extractBack(GapBuf* buf)
{
  ...
  char* text = (char*)malloc(gb_back(buf) + 1);
  if (!text) return NULL;

  strncpy(text, buf->buffer + buf->gap_end, gb_back(buf));
  text[gb_back(buf)+1] = '\0';
