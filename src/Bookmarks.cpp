...
namespace SimpleXTree
{
	...
	BItem::BItem(std::wstring const& name, std::wstring const& path) : Name(name), Path(path)
	{
	}

	Bookmarks::Bookmarks() : m_activated(false), m_selected(0), m_downPressed(false), m_upPressed(false), m_escPressed(false), m_enterPessed(false)
	{
		...
		StrUtil::Replace(p, L"SimpleXTree.exe", L"Bookmarks.txt");
		...
		if (ifile) {
			...
			if (ifile.is_open())
			{
				...
				while (std::getline(ifile, line))
				{
					...
					Items.push_back(BItem(tokens[0], tokens[1]));
			...
			//Items.push_back(BItem(L"Notepad", L"%windir%\\system32\\notepad.exe"));
			int a = 1;
			a++;
		}
	...
	void Bookmarks::GoToL(BItem const& item)
	{
		GoTo(item.Path);
		m_activated = false;
	}
